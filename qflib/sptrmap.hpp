...
#include <qflib/exception.hpp> // should include here
#include <map>
#include <string>
#include <vector>
#include <cctype>
...
template<typename T>
bool SPtrMap<T>::contains(std::string const& name) const {
    std::string nm = processName(name);
    return map_.find(nm) != map_.end();
}

template<typename T> 
typename SPtrMap<T>::ptr_type 
...
SPtrMap<T>::set(std::string const& name, ptr_type sp) {
    ...
    auto it = map_.find(nm);
    if (it != map_.end()) {
        ver = it->second.second;
        it->second = std::make_pair(sp, ++ver);
    } else {
        map_.insert(std::make_pair(nm, std::make_pair(sp, ++ver)));
    }
    return std::make_pair(nm, ver);
...
template<typename T>
unsigned long SPtrMap<T>::version(std::string const& name) const {
    std::string nm = processName(name);
    return get_pair(nm).second;
}

template<typename T>
void SPtrMap<T>::clear() { 
    map_.clear(); 
    return;
}
